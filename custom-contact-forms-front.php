...
if (!class_exists('CustomContactFormsFront')) {
   class CustomContactFormsFront extends CustomContactForms {
      ...
      function insertFrontEndStyles() {
            wp_register_style('CCFStandardsCSS', plugins_url() . '/kip/css/custom-contact-forms-standards.css');
              wp_register_style('CCFFormsCSS', plugins_url() . '/kip/css/custom-contact-forms.css');
      ...
      function insertFrontEndScripts() {
         ...
         wp_enqueue_script('ccf-main', plugins_url() . '/kip/js/custom-contact-forms.js', '1.0');
      }

      function insertDatePickerScripts() {
         wp_enqueue_script('jquery-ui-datepicker', plugins_url() . '/kip/js/jquery.ui.datepicker.js', array('jquery-ui-core', 'jquery-ui-widget'));
         wp_enqueue_script('jquery-ui-datepicker-id', plugins_url() . '/kip/js/jquery.ui.datepicker-ui.js', array('jquery-ui-datepicker'));
         wp_enqueue_script('ccf-datepicker', plugins_url() . '/kip/js/custom-contact-forms-datepicker.js', '1.4');
      }
      ...
      function shortCodeToForm($atts) {
         ...
         if ($form == 'question_dashboard') {
            $form = CustomContactFormsDB::selectForm(null, 'question');
            $form = $form->id;
            $this_form = parent::selectForm($form);
            return $this->getFormCodeAdmin($this_form);
         } else {
            $form = CustomContactFormsDB::selectForm(null, $form);
            $form = $form->id;
            $this_form = parent::selectForm($form);
      ...
      function shortCodeToDisplayPublished() {
         ccf_utils::load_module('export/custom-contact-forms-user-data.php');
         $user_data_array_act = parent::selectAllQuestions();
         $user_data_array_blm_jwb = parent::selectAllUserData(FORM_ID_QUESTION);

         $user_data_array = array();

         foreach ($user_data_array_act as $data_object) {
            $data_object->blm_terjawab = false;
            $user_data_array[$data_object->data_time] = $data_object;
         }

         foreach ($user_data_array_blm_jwb as $data_object) {
            $obj = new stdClass();
            $obj->data_formid = $data_object->data_formid;
            $obj->data_time = $data_object->data_time;
            $obj->form_page = $data_object->form_page;
            $obj->encoded_data = $data_object->data_value;
            $obj->question_data = $data_object->data_value;
            $obj->blm_terjawab = true;

            $a = new DateTime('@' . $obj->data_time);
            $b = new DateTime('@' . time());
            $interval = $b->diff($a);

            $obj->interval =  $interval->format('%a hari %h jam %i menit')."\n";

            $user_data_array[$data_object->data_time] = $obj;
         }

         krsort($user_data_array);
         ...
         $out .= "
               <script>
               jQuery('.show-answer').live('click', function(e){
                  jQuery(this).closest('tr').next().toggle();
               })
               </script>
               ";
         $out .= '<table cellspacing="0" class="wp-list-table widefat fixed posts">
   <thead>
   <tr>
      <th class="manage-column column-title sortable desc">No</th>
      <th class="manage-column column-title sortable desc">Penanya/Alamat</th>
      <th class="manage-column column-title sortable desc">Subyek/Pertanyaan</th>
      <th class="manage-column column-title sortable desc">Status/Jawaban</th>
         ...
         foreach ($user_data_array as $data_object) {
            ...
            $user = parent::selectFixUser($data_object->user_id);
            $data_user = new CustomContactFormsUserData(array('encoded_data' => $user->fix_data));
            $data_user_array = $data_user->getDataArray();
            $dataUser = get_userdata($data_object->user_id);
            $out .= "<tr class=\"dqa\">";
            $out .= "<td>" . $i . "</td>";
            $out .= "<td>" . $data_user_array['namalengkap'] . " / " . $data_user_array['alamat'] . "</td>";
            $out .= "<td><b><i>" . $data_array['subject'] . "</b></i> <br/> " . $data_array['question'] . "</td>";

            if ($data_object->is_publish) {
               $out .= "<td><a href=\"javascript:;\" class=\"show-answer\" title=\"lihat jawaban\"><b><i>terjawab</i></b></a></td>";
            } else {
               if ($data_object->is_rejected) {
                  $out .= "<td><a href=\"javascript:;\" title=\"ditolak\"><b><i>terjawab</i></b></a></td>";
               } else if ($data_object->blm_terjawab) {
                  $out .= "<td><a href=\"javascript:;\" title=\"belum terjawab\"><b><i>belum terjawab ( " . $data_object->interval. ")</i></b></a></td>";
               } else {
                  $out .= "<td><a href=\"javascript:;\" title=\"tertutup(tidak diterbitkan)\"><b><i>terjawab</i></b></a></td>";
               }
            }

            $out .= "</tr>";
            if ($data_object->is_publish) {
            $out .= "<tr class=\"dqa\" style=\"display:none\">";
            $out .= '<td class="colspanchange" colspan="4">

            <fieldset class="inline-edit-col-left">
                  <div class="inline-edit-col">
                  <label>
            <span class="title"><b><i>Dijawab pada : </i></b></span><br/>
            <span class="input-text-wrap">
                  ' . nl2br(esc_html(date('d F, Y H:i:s', $data_object->action_time))) . '
            </span><br/>
                  <span class="tile"><b><i>Cara Memperoleh Informasi : </i></b></span><br/>
            <span class="input-text-wrap">
                  ' . nl2br(isset($data_array['source']) ? $data_array['source'] : 'Lihat di Web') . '
            </span><br/>
            <span class="title"><b><i>Jawaban : </i></b></span><br/>
            <span class="input-text-wrap">
                  ' . nl2br($data_object->answer_data) . '
            </span>
         </label>
                  </div>
         </fieldset>
      ...
      function insertFormSuccessCodeAdmin() {
         $admin_options = parent::getAdminOptions();
         if ($this->current_form !== 0) {
            $form = parent::selectForm($this->current_form);
            $success_message = (!empty($form->form_success_message)) ? $form->form_success_message : $admin_options['form_success_message'];
         ...
         //if ($form->form_style != 0) {
            ...
            ?>
                  ...
                  <script>
                  jQuery('#ccf-form-success .close').live('click', function(e){
                     jQuery('#ccf-form-success').remove();
                  });
                  </script>
                      <?php
               //}
            ?>
                 <div id="ccf-form-success" style="display:block">
                     <div>
                        <h5><?php echo esc_html($success_title); ?></h5>
                         <a href="javascript:void(0)" class="close">&times;</a>
                      </div>
      ...
      function getFormCode($form, $is_widget_form = false) {
         ...
         foreach ($fields as $field_id) {
            ...
            if (isset($_SESSION['ccf_fields'][$field->field_slug])) {
               ...
               if ($field->field_slug == 'user_id') {
                  $hiddens .= '<input type="hidden" name="'.ccf_utils::decodeOption($field->field_slug, 1, 1).'" value="'. (is_user_logged_in() ? get_current_user_id() : 0) .'" '.$input_id.''.$code_type.'>' . "\n";
               } else {
         ...
         if ($form->id == FORM_ID_REGISTERED) {
            $tos .= '<br/><br/><div style="float:none;padding-top:20px;clear:both;max-width:100%!important">Dengan mengklik tombol "Daftar" di bawah, saya menyatakan bahwa informasi yang saya isikan di atas adalah <b>BENAR</b>.<br/>
            Pengisian yang salah dan tidak lengkap akan menyebabkan permohonan data Anda <b>TIDAK AKAN DIPROSES</b><br/>
            Username dan password untuk mengakses situs ini akan dikirimkan ke <b>EMAIL</b> Anda</div>';
         }

         $out .= '<input name="form_page" value="'.esc_url($_SERVER['REQUEST_URI']).'" type="hidden"'.$code_type.'>'."\n".'<input type="hidden" name="fid" value="'.esc_attr($form->id).'"'.$code_type.'>'."\n".$hiddens."\n".' ' . $tos . ' <input type="submit" id="submit-' . esc_attr($form->id) . '-'.$form_key.'" class="button-primary" value="' . $submit_text . '" name="customcontactforms_submit"'.$code_type.'>';
      ...
      function getFormCodeAdmin($form, $is_widget_form = false) {
         $this->error_return = false;
         echo $this->contentFilter("");
         if (empty($form)) return '';
         $admin_options = parent::getAdminOptions();
         $form_key = time();
         $out = '';
         ...
         foreach ($fields as $field_id) {
            ...
            if (isset($_SESSION['ccf_fields'][$field->field_slug])) {
               ...
               $maxlength = (empty($field->field_maxlength) or $field->field_maxlength <= 0) ? '' : ' maxlength="'.esc_attr($field->field_maxlength).'"';
               $out .= '<div class="input-text-wrap">
         <label style="padding-top: 3px;padding-left: 3px;" id="text-wrap" for="'.ccf_utils::decodeOption($field->field_slug, 1, 1).'" class="prompt">' .ccf_utils::decodeOption($field->field_label, 1, 1).'</label>
         <input style="width:100%" id="ccf-text" title="' . ccf_utils::decodeOption($field->field_label, 1, 1). '" class="text-wrap" autocomplete="off" '.$input_id.' type="text" name="'.ccf_utils::decodeOption($field->field_slug, 1, 1).'" value="'.  $field_value.'"'.$maxlength.''.$code_type.'>
      </div><br/>';
               //$out .= '<div>'."\n".'<label for="'.ccf_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccf_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<input class="'.esc_attr($field->field_class).' '.$tooltip_class.'" '.$instructions.' '.$input_id.' type="text" name="'.ccf_utils::decodeOption($field->field_slug, 1, 1).'" value="'.$field_value.'"'.$maxlength.''.$code_type.'>'."\n".'</div>' . "\n";
            } elseif ($field->field_type == 'File') {
               $file_upload_form = ' enctype="multipart/form-data" ';
               ...
               $out .= '<div class="input-text-wrap">
         <label style="padding-top: 3px;padding-left: 3px;" id="text-wrap" for="'.ccf_utils::decodeOption($field->field_slug, 1, 1).'" class="prompt">' .ccf_utils::decodeOption($field->field_label, 1, 1).'</label>
         <textarea style="width:100%;height:100%" rows="5" cols="15" id="ccf-text" title="' . ccf_utils::decodeOption($field->field_label, 1, 1). '" class="text-wrap" autocomplete="off" '.$input_id.' type="text" name="'.ccf_utils::decodeOption($field->field_slug, 1, 1).'"' . $code_type.'>' . $field_value . '</textarea>
      </div>';
...
//                $out .= '<div>'."\n".'<label for="'.ccf_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccf_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<textarea class="'.esc_attr($field->field_class).' '.$tooltip_class.'" '.$instructions.' '.$input_id.' rows="5" cols="40" name="'.ccf_utils::decodeOption($field->field_slug, 1, 1).'">'.$field_value.'</textarea>'."\n".'</div>' . "\n";
         ...
         $out = '<form id="'.$form_id.'" method="'.esc_attr($form_method).'" action="'.esc_url($action).'" class="'.esc_attr($style_class).'"'.$file_upload_form.'>' . "\n" . $out;
         $submit_text = (!empty($form->submit_button_text)) ? ccf_utils::decodeOption($form->submit_button_text, 1, 0) : __('Submit', 'custom-contact-forms');
         $out .= '<input name="form_page" value="'.esc_url($_SERVER['REQUEST_URI']).'" type="hidden"'.$code_type.'>'."\n".'<input type="hidden" name="fid" value="'.esc_attr($form->id).'"'.$code_type.'>'."\n".$hiddens."\n".'<span id="publishing-action"><input type="submit" id="submit-' . esc_attr($form->id) . '-'.$form_key.'" class="button-primary" value="' . $submit_text . '" name="customcontactforms_submit"'.$code_type.'></span>';
         if (!empty($add_reset)) $out .= $add_reset;
         $out .= "\n" . '</form>';
      ...
      function processFileUpload($field) {
         ...
         if (!is_dir(ABSPATH."wp-content/uploads/kip")) {
            mkdir(ABSPATH."wp-content/uploads/kip");
            file_put_contents(ABSPATH."wp-content/uploads/kip/index.php", "<?php");
         }

         // create necessary directories
         if (!is_dir(ABSPATH."wp-content/uploads/kip/".date("Y"))) {
            mkdir(ABSPATH."wp-content/uploads/kip/".date("Y"));
            file_put_contents(ABSPATH."wp-content/uploads/kip/".date("Y") . "/index.php", "<?php");
         }
         if (!is_dir(ABSPATH . "wp-content/uploads/kip/".date("Y")."/".date("m"))) {
            mkdir(ABSPATH . "wp-content/uploads/kip/".date("Y")."/".date("m"));
            file_put_contents(ABSPATH."wp-content/uploads/kip/".date("Y") . "/" . date("m") . "/index.php", "<?php");
         }
         ...
         $file_name_addon = ".";
         $i = 1;
         while (file_exists( ABSPATH . "wp-content/uploads/kip/".date("Y")."/".date("m")."/" . $file_name . $file_name_addon . $ext)) {
            $file_name_addon = ' ('.$i.').';
            $i++;
         }
         $target_path = ABSPATH . "wp-content/uploads/kip/".date("Y")."/".date("m")."/" . $file_name . $file_name_addon . $ext;
         $this->form_uploads[$field->field_slug] = get_site_url() . "/wp-content/uploads/kip/".date("Y")."/".date("m")."/" . $file_name . $file_name_addon . $ext;
      ...
      function processForms() {
         if (isset($_POST['ccf_customhtml']) || isset($_POST['customcontactforms_submit'])) {
            ...
            $lang = array();
            $lang['field_blank'] = __('Isian berikut tidak boleh kosong: ', 'custom-contact-forms');
...
//                   $mail->Send();
               ...
               add_filter('admin_footer_text', array(&$this, 'insertFormSuccessCode'), 1);
            }
            unset($_POST);
            ...
            foreach ($fields as $field_id) {
               $field = parent::selectField($field_id, '');
               $this->validateMyField($field, $_POST);
            ...
            if (empty($errors)) {
               ...
               //do email to user register with password
               add_filter('wp_mail_content_type',create_function('', 'return "text/html";'));

               if ($form->id == FORM_ID_REGISTERED) {
                  $html = "<h2>Pendaftaran Baru</h2>
                  <p>" . "Admin KIP, " . ", terdapat 1 pendaftaran terhadap situs, dengan data : </p>
                  <p>" . $body . "</p>
                        <p>Silakan <a href='" . get_option('siteurl') . '/wp-admin'. "'>dasbor situs</a> untuk lebih detil.</p>";
                  ...
                  $mail->Subject = "KIP - Pendaftaran Baru";
                  $mail->CharSet = "utf-8";
                  $mail->MsgHTML($body);
                  $mail->Send();
               ...
               if ($form->id == FORM_ID_QUESTION) {
                  $html = "<h2>Pertanyaan Baru</h2>
                  <p>" . "Admin KIP, " . ", terdapat 1 pertanyaan pada situs, dengan data : </p>
                  <p>" . $body . "</p>
                        <p>Silakan kunjungi <a href='" . get_option('siteurl') . '/wp-admin/admin.php?page=ccf-pending'. "'>dasbor situs</a> untuk lebih detil.</p>";
                  ...
                  $mail->Subject = "KIP - Pertanyaan Baru";
                  $mail->CharSet = "utf-8";
                  $mail->MsgHTML($body);
                  $mail->Send();
...
//                   $mail->Send();
               ...
               add_action('wp_footer', array(&$this, 'insertFormSuccessCode'), 1);
               add_filter('admin_footer_text', array(&$this, 'insertFormSuccessCodeAdmin'), 1);
            }
            unset($_POST);
            $_POST = array();
         }
      ...
      function validateMyField($field, $post) {
         if ($field->field_slug == 'username') {
            if (!validate_username($post['username'])) {
               $this->setFormError('username', __('Username not valid', 'custom-contact-forms'));
               return false;
            }

            //check username exist
            if (username_exists($post['username'])) {
               $this->setFormError('username', __('Username already used', 'custom-contact-forms'));
               return false;
            }
         }
         if ($field->field_slug == 'fixedEmail') {

            if (!$this->validEmail($post['fixedEmail'])) {
               if (empty($field->field_error))
                  $this->setFormError('fixedEmail', __('The email address you provided is not valid.', 'custom-contact-forms'));
               else $this->setFormError('fixedEmail', $field->field_error);
            }

            if (!is_email($post['fixedEmail'])) {
               $this->setFormError('fixedEmail', __("Email is not valid", 'custom-contact-forms'));
               return false;
            }


            if (email_exists($post['fixedEmail'])) {
               $this->setFormError('fixedEmail', __("Email already used", 'custom-contact-forms'));
               return false;
            }
      ...
      function getCaptchaCode($field_object, $form_id) {
         ...
         $out = '<img width="96" height="24" alt="' . __('Captcha image for Custom Contact Forms plugin. You must type the numbers shown in the image', 'custom-contact-forms') . '" id="captcha-image" src="' . get_bloginfo('wpurl') . '/wp-content/plugins/kip/image.php?fid='.$form_id.'"'.$code_type.'>
