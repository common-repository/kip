...
if (!class_exists('ccf_utils')) {
   class ccf_utils {
      ...
      function defineConstants() {
         ...
         define('CCF_FIX_DATA_USER_TABLE', $prefix . 'customcontactforms_fix_data_user');
         define('CCF_QUESTION_TABLE', $prefix . 'customcontactforms_question');
         ...
         define('CCF_BASE_PATH', ABSPATH . 'wp-content/plugins/kip/');
         ...
         if (!defined('FORM_ID_REGISTERED') || FORM_ID_REGISTERED == 'FORM_ID_REGISTERED') {
            ccf_utils::load_module('db/custom-contact-forms-db.php');
            $regist = CustomContactFormsDB::selectForm(null, 'registration');
            if ($regist)
               define('FORM_ID_REGISTERED', $regist->id);
         }

         if (!defined('FORM_ID_QUESTION') ||  FORM_ID_QUESTION == 'FORM_ID_QUESTION') {
            ccf_utils::load_module('db/custom-contact-forms-db.php');
            $qa = CustomContactFormsDB::selectForm(null, 'question');
            if ($qa)
               define('FORM_ID_QUESTION', $qa->id);
         }
