   ...
   class CustomContactFormsAdmin extends CustomContactForms {
      ...
      function isPluginAdminPage() {
         $pages = array (
               ...
               'ccf-settings',
               'ccf-saved-form-submissions',
               'ccf-pending',
               'ccf-pending-question',
               'ccf-answered-question',
               'ccf-fields',
//                'ccf-new-question',
               'custom-contact-data',
               'ccf-user',
      ...
      function downloadExportFile() {
         if (isset($_POST['ccf_export'])) {
            ...
            ccf_utils::redirect(plugins_url() . '/kip/download.php?location=export/' . $file);
         }
      }
...
<?php
      ...
      function insertBackEndStyles() {
            wp_register_style('ccf-standards', plugins_url() . '/kip/css/custom-contact-forms-standards.css');
            wp_register_style('ccf-jquery-ui', plugins_url() . '/kip/css/jquery-ui.css');
            wp_register_style('ccf-hover', plugins_url() . '/kip/css/jquery.fancybox.css');
            wp_register_style('ccf-admin', plugins_url() . '/kip/css/custom-contact-forms-admin.css');

         wp_register_style('ccf-my-css', plugins_url() . '/kip/css/custom-contact-forms.css');

         wp_register_style('ccf-colorpicker', plugins_url() . '/kip/css/colorpicker.css');
            wp_enqueue_style('ccf-jquery-ui');
         wp_enqueue_style('ccf-standards');
         wp_enqueue_style('ccf-my-css');
         wp_enqueue_style('ccf-admin');
         wp_enqueue_style('ccf-hover');
      ...
      function insertAdminScripts() {
         ...
         $js_ajax = array('plugin_dir' => plugins_url() . '/kip',
                     'url' => admin_url('admin-ajax.php'),
                     'nonce' => wp_create_nonce('ccf-ajax-nonce'));
         wp_enqueue_script('jquery');
         wp_enqueue_script('my-ccf', plugins_url() . '/kip/js/my-javascript.js', $js_version);
         wp_deregister_script('jquery-form');
         wp_register_script('jquery-form', plugins_url() . '/kip/js/jquery.form.js', $js_version);
         wp_enqueue_script('jquery-ui-core');
         wp_enqueue_script('jquery-ui-tabs');
         wp_enqueue_script('jquery-ui-dialog');
         wp_enqueue_script('jquery-ui-sortable');
         wp_enqueue_script('jquery-tools', plugins_url() . '/kip/js/jquery.tools.min.js');
         wp_enqueue_script('jquery-ui-widget', plugins_url() . '/kip/js/jquery.ui.widget.js');
         wp_enqueue_script('ccf-hover', plugins_url() . '/kip/js/jquery.fancybox.pack.js', $js_version);
         wp_enqueue_script('ccf-admin-inc', plugins_url() . '/kip/js/custom-contact-forms-admin-inc.js', $js_version);
         wp_enqueue_script('ccf-admin', plugins_url() . '/kip/js/custom-contact-forms-admin.js', $js_version);
         if ($admin_options['admin_ajax'] == 1) {
            wp_enqueue_script('ccf-admin-ajax', plugins_url() . '/kip/js/custom-contact-forms-admin-ajax.js', array('jquery-form'), $js_version);
      ...
      function handleAdminPostRequests() {
         ...
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'approve-pending') {
            //approve user
            $object_id = $_POST['object_id'];
            $data = parent::selectUserData($object_id);
            ccf_utils::load_module('export/custom-contact-forms-user-data.php');
            $dataString = $data->data_value;
            $data = new CustomContactFormsUserData(array('encoded_data' => $data->data_value));
            $data_array= $data->getDataArray();
            $dataReg = parent::approveUser($data_array['username'], $data_array['fixedEmail'], $data_array['namalengkap']);

            $html = "<h2>Registrasi Disetujui</h2>
            <p>" . $data_array['username'] . ", selamat registrasi anda berhasil</p>
            <p>Silakan login dengan <br />
            username : " . $data_array['username'] . " <br/>
            password : " . $dataReg[1] . "</p>
         <p>Silakan <a href='" . get_option('siteurl') . '/wp-admin'. "'>login</a> untuk lebih detil.</p>";
            ...
            $mail->AddAddress($data_array['fixedEmail']);

            $mail->Subject = "KIP - Registrasi Anda Disetujui";
            $mail->CharSet = "utf-8";
            $mail->MsgHTML($body);
            $mail->Send();

            file_put_contents('/tmp/wp-kip.log', $html . PHP_EOL . PHP_EOL, FILE_APPEND);



            parent::saveFixData($dataReg[0], $dataString);
            parent::deleteUserData($object_id);
         }

         //save question answered
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'save-q') {
            //approve user
            $object_id = $_POST['object_id'];
            $answer    = $_POST['answer'];
            $source    = $_POST['source'];
            ...
            //save answer
            parent::insertQuestions($data_array['user_id'], $dataTime, $dataString, $answer, $source, "", 0, 0);
            parent::deleteUserData($object_id);

            //mail to user
            $user_info = get_userdata($data_array['user_id']);

            $html = "<h2>Pertanyaan anda terjawab,</h2>
               <p>Pertanyaan anda : \"" . $data_array['question'] . "\", telah dijawab oleh admin.</p>
               <p>Silakan kunjungi <a href='" . get_option('siteurl') . '/wp-admin'. "'>dasbor situs</a> untuk lebih detil.</p>";
            ...
            $mail->Subject = "KIP - Pertanyaan anda terjawab";
            $mail->CharSet = "utf-8";
            $mail->MsgHTML($body);
            $mail->Send();
         ...
         //publish question answered
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'publish-q') {
            //approve user
            $object_id = $_POST['object_id'];
            $answer    = $_POST['answer'];
            $source    = $_POST['source'];
            ...
            //save answer
            parent::insertQuestions($data_array['user_id'], $dataTime, $dataString, $answer, $source, "", 1, 0);
            parent::deleteUserData($object_id);
         ...
         //reject question answered
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'delete-q') {
            //approve user
            $object_id = $_POST['object_id'];
            $answer    = $_POST['answer'];
            ...
            //save answer
            parent::insertQuestions($data_array['user_id'], $dataTime, $dataString, $answer, "", "", 0, 1);
            parent::deleteUserData($object_id);

            $user_info = get_userdata($data_array['user_id']);

            $html = "<h2>Pertanyaan anda ditolak,</h2>
               <p>Pertanyaan anda : \"" . $data_array['question'] . "\", telah ditolak oleh admin.</p>
               <p>Silakan kunjungi <a href='" . get_option('siteurl') . '/wp-admin'. "'>dasbor situs</a> untuk lebih detil.</p>";
            ...
            $mail->Subject = "KIP - Pertanyaan anda ditolak";
            $mail->CharSet = "utf-8";
            $mail->MsgHTML($body);
            $mail->Send();
         ...
         //reject question answered
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'topending-a') {
            //approve user
            $object_id = $_POST['object_id'];

            $data = parent::selectQuestion($object_id);

            ccf_utils::load_module('export/custom-contact-forms-user-data.php');
            $dataForm = new CustomContactFormsUserData(array('encoded_data' => $data->question_data));


            //insert to user data
            $data_object = new
               CustomContactFormsUserData(
                  array(
                     'data_array' => $dataForm->getDataArray(),
                     'form_page' => "",
                     'form_id' => FORM_ID_QUESTION,
            ...
            //delete
            parent::deleteQuestion($object_id);
         }


         //update answer
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'updateans-a') {
            //approve user
            $object_id = $_POST['object_id'];
            $answer    = $_POST['answer'];
            $source    = $_POST['source'];
            ...
            $data = parent::selectQuestion($object_id);
            parent::updateQuestions($data->id, $answer, $source, $data->reject_data, $data->is_publish, $data->is_rejected);

         }

         //unpublish answer
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'unpublished-a') {
            //approve user
            $object_id = $_POST['object_id'];

            $data = parent::selectQuestion($object_id);
            parent::updateQuestions($data->id, $data->answer_data, $data->info_source, $data->reject_data, 0, $data->is_rejected);

         }

         //publish answer
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'publish-a') {
            //approve user
            $object_id = $_POST['object_id'];

            $data = parent::selectQuestion($object_id);
            parent::updateQuestions($data->id, $data->answer_data, $data->info_source, $data->reject_data, 1, $data->is_rejected);

         }

         //set ppid
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'set-ppid') {
            //approve user
         ...
         //set subscriber
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'set-sub') {
            //approve user
            $userId = $_POST['user_id'];
         ...
         //set subscriber
         if (isset($_POST['type_action']) &&  $_POST['type_action'] == 'del-user') {
            //approve user
            $userId = $_POST['user_id'];
...
<form class="rate-me" action="https://www.paypal.com/cgi-bin/webscr" method="post">
   <input type="hidden" name="cmd" value="_s-xclick"> <input type="hidden" name="hosted_button_id" value="TXYVDCH955V28"> <a href="http://wordpress.org/extend/plugins/custom-contact-forms" title="<?php _e("Rate This Plugin", 'custom-contact-forms'); ?>">
                <?php _e("We need your help to continue development! Please <span>rate this plugin</span> to show your support.", 'custom-contact-forms'); ?></a> <input type="image" src="https://www.paypal.com/en_US/i/btn/btn_donate_SM.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!"> <img alt="Donate to Custom Contact Forms plugin" border="0" src="https://www.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">
</form>
<?php
      }
...
<div id="customcontactforms-admin">
   <div class="">
      <h2>
               <?php _e("KIP Forms", 'custom-contact-forms'); ?>
            </h2>
   </div>
   ...
   <div id="ccf-tabs">
      <ul id="plugin-nav">
         ...
         <li><a href="#styles"><?php _e("Tampilan", 'custom-contact-forms'); ?></a></li>
         <li><a href="#advanced"><?php _e("Lanjutan", 'custom-contact-forms'); ?></a></li>
      </ul>
         ...
         <h3 class="manage-h3">
            <?php _e("Kelola Form KIP", 'custom-contact-forms'); ?>
           </h3>
            ...
            <table class="widefat post" id="manage-forms" cellspacing="0">
               <thead>
                  <tr>
                     ...
                     <th scope="col" class="manage-column form-code"><?php _e("Shortcode", 'custom-contact-forms'); ?></th>
                     <th scope="col" class="manage-column form-slug"><?php _e("Slug", 'custom-contact-forms'); ?></th>
                     <th scope="col" class="manage-column form-title"><?php _e("Judul", 'custom-contact-forms'); ?></th>
                     <th scope="col" class="manage-column form-style"><?php _e("Tampilan", 'custom-contact-forms'); ?></th>
         ...
         ?>
              <tr class="row-form-<?php echo esc_attr($forms[$i]->id); ?> <?php if ($i % 2 == 0) echo 'ccf-evenrow'; ?>">
                     ...
                     <td>[custom form=<?php echo esc_attr($forms[$i]->id == FORM_ID_REGISTERED ? 'registration' : 'question'); ?>]</td>
                     <td><?php echo esc_attr($forms[$i]->form_slug); ?></td>
                  ...
                  <?php echo $sty_opt; ?>
                     ...
                     <td><input class="object-id" type="hidden" name="objects[<?php echo esc_attr($forms[$i]->id); ?>][object_id]" value="<?php echo esc_attr($forms[$i]->id); ?>" /> <input type="hidden" class="object-type" name="objects[<?php echo esc_attr($forms[$i]->id); ?>][object_type]" value="form" /> <input type="button" class="single-save" value="<?php _e('Simpan', 'custom-contact-forms'); ?>" /> <input type="button" class="single-delete" value="<?php _e('Hapus', 'custom-contact-forms'); ?>" /> <input type="button" class="form-options-expand-link" value="<?php _e('Opsi', 'custom-contact-forms'); ?>" />
                        <div class="loading-img-container">
                           <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" width="16" height="16" class="ccf-hide loading-img-inner-form-<?php echo esc_attr($forms[$i]->id); ?>" />
                        </div></td>
                  </tr>
                  <tr class="row-form-<?php echo esc_attr($forms[$i]->id); ?> <?php if ($i % 2 == 0) echo 'ccf-evenrow'; ?>">
                     ...
                     ?></ul> <input name="objects[<?php echo esc_attr($forms[$i]->id); ?>][values][form_access_update]" type="hidden" value="1" /></li>
                              <li></li>
                           </ul>
                        </div>
                        <div class="right">
      ...
      echo esc_html($this_field->field_slug);?> (<?php echo esc_html($this_field->field_type);?>)</li>
               ...
               echo '</ul>';
           ?>
                              <input class="attached-update-button" type="button" value="<?php _e('Save Field Configuration', 'custom-contact-forms'); ?>" /> <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" width="16" height="16" class="ccf-hide loading-img-field-config-form-<?php echo esc_attr($forms[$i]->id); ?>" />
         ...
         ?>
            ...
            </select> <input type="submit" name="object_bulk_apply" class="bulk-apply" value="<?php _e('Apply', 'custom-contact-forms'); ?>" /> <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" width="16" height="16" class="loading-img" />
         </form>
      </div>
         ...
         <div id="create-styles" class="postbox">
            ...
            <div class="inside">
               ...
               <form id="ccf-create-style" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
                  ...
                  <ul class="style_left">
                     <li><label for="style_slug">*
                  <?php _e("Style Slug:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="30" class="ccf-width75" name="object[style_slug]" />
                  <?php _e("(Must be unique)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="title_fontsize">
                  <?php _e("Title Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="1.2em" class="ccf-width75" name="object[title_fontsize]" />
                  <?php _e("(ex: 10pt, 10px, 1em)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="title_fontcolor">
                  <?php _e("Title Font Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="333333" class="ccf-width75 colorfield" name="object[title_fontcolor]" />
                  <?php _e("(ex: FF0000)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="label_width">
                  <?php _e("Label Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="200px" class="ccf-width75" name="object[label_width]" />
                  <?php _e("(ex: 100px or 20%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="label_fontsize">
                  <?php _e("Label Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="1em" class="ccf-width75" name="object[label_fontsize]" />
                  <?php _e("(ex: 10px, 10pt, 1em)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="label_fontcolor">
                  <?php _e("Label Font Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="333333" class="ccf-width75 colorfield" name="object[label_fontcolor]" />
                  <?php _e("(ex: FF0000)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="input_width">
                  <?php _e("Text Field Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="200px" class="ccf-width75" name="object[input_width]" />
                  <?php _e("(ex: 100px or 100%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="textarea_width">
                  <?php _e("Textarea Field Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="200px" class="ccf-width75" name="object[textarea_width]" />
                  <?php _e("(ex: 100px or 100%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="textarea_height">
                  <?php _e("Textarea Field Height:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="90px" class="ccf-width75" name="object[textarea_height]" />
                  <?php _e("(ex: 100px or 100%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="field_fontsize">
                  <?php _e("Field Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="1.3em" class="ccf-width75" name="object[field_fontsize]" />
                  <?php _e("(ex: 10px, 10pt, 1em", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="field_fontcolor">
                  <?php _e("Field Font Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="333333" class="ccf-width75 colorfield" name="object[field_fontcolor]" />
                  <?php _e("(ex: 333333)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="field_borderstyle">
                  <?php _e("Field Border Style:", 'custom-contact-forms'); ?>
                  </label> <select class="ccf-width75" name="object[field_borderstyle]">
                    <?php echo str_replace('<option>solid</option>', '<option selected="selected">solid</option>', $border_style_options); ?>
                  </select></li>
                     <li><label for="form_margin">
                  <?php _e("Form Margin:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="7px" class="ccf-width75" name="object[form_margin]" />
                  <?php _e("(ex: 5px or 1em)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="label_margin">
                  <?php _e("Label Margin:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="5px" class="ccf-width75" name="object[label_margin]" />
                  <?php _e("(ex: 5px or 1em)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="textarea_backgroundcolor">
                  <?php _e("Textarea Background Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="f5f5f5" class="ccf-width75 colorfield" name="object[textarea_backgroundcolor]" />
                  <?php _e("(ex: FF0000)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="success_popover_fontcolor">
                  <?php _e("Success Popover Font Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="333333" class="ccf-width75 colorfield" name="object[success_popover_fontcolor]" />
                  <?php _e("(ex: 333333)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="success_popover_title_fontsize">
                  <?php _e("Success Popover Title Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="15px" class="ccf-width75" name="object[success_popover_title_fontsize]" />
                  <?php _e("(ex: 12px, 1em, 100%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="form_backgroundcolor">
                  <?php _e("Form Background Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="ffffff" class="ccf-width75 colorfield" name="object[form_backgroundcolor]" />
                  <?php _e("(ex: 12px, 1em, 100%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="tooltip_backgroundcolor">
                  <?php _e("Tooltip Background Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="000000" class="ccf-width75 colorfield" name="object[tooltip_backgroundcolor]" />
                  <?php _e("(ex: 000000 or black)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="form_borderwidth">
                  <?php _e("Form Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="100%" class="ccf-width75" name="object[form_width]" />
                  <?php _e("(ex: 100px or 50%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="input_width">
                  <?php _e("Field Border Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="999999" class="ccf-width75 colorfield" name="object[field_bordercolor]" />
                  <?php _e("(ex: 100px or 100%)", 'custom-contact-forms'); ?>
                 </li>
                  </ul>
                  <ul class="style_right">

                     <li><label for="form_borderstyle">
                  <?php _e("Form Border Style:", 'custom-contact-forms'); ?>
                  </label> <select class="ccf-width75" name="object[form_borderstyle]">
                    <?php echo $border_style_options; ?>
                  </select></li>
                     <li><label for="form_bordercolor">
                  <?php _e("Form Border Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="ffffff" class="ccf-width75 colorfield" name="object[form_bordercolor]" />
                  <?php _e("(ex: 000000)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="form_borderwidth">
                  <?php _e("Form Border Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="0px" class="ccf-width75" name="object[form_borderwidth]" />
                  <?php _e("(ex: 1px)", 'custom-contact-forms'); ?>
                 </li>

                     <li><label for="form_borderwidth">
                  <?php _e("Form Font Family:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="120" value="Verdana, tahoma, arial" class="ccf-width75" name="object[form_fontfamily]" />
                  <?php _e("(ex: Verdana, Tahoma, Arial)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="submit_width">
                  <?php _e("Button Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="auto" class="ccf-width75" name="object[submit_width]" />
                  <?php _e("(ex: 100px, 30%, auto)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="submit_height">
                  <?php _e("Button Height:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="30px" class="ccf-width75" name="object[submit_height]" />
                  <?php _e("(ex: 100px or 30%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="submit_background">
                  <?php _e("Button Background:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="200" value="http://" class="ccf-width175" name="object[submit_background]" />
                  <?php _e("(any URL)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="submit_background_repeat">
                  <?php _e("Button Background Repeat:", 'custom-contact-forms'); ?>
                  </label> <select name="objects[<?php echo esc_attr($style->id); ?>][values][submit_background_repeat]">
                           <option value="no-repeat">No Repeat</option>
                           <option value="repeat-x">Repeat X</option>
                           <option value="repeat-y">Repeat Y</option>
                           <option value="repeat">Repeat Both</option>
                     </select></li>
                     <li><label for="submit_fontsize">
                  <?php _e("Button Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="1.1em" class="ccf-width75" name="object[submit_fontsize]" />
                  <?php _e("(ex: 10px, 10pt, 1em)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="submit_fontcolor">
                  <?php _e("Button Font Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="333333" class="ccf-width75 colorfield" name="object[submit_fontcolor]" />
                  <?php _e("(ex: FF0000)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="field_backgroundcolor">
                  <?php _e("Field Background Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="f5f5f5" class="ccf-width75 colorfield" name="object[field_backgroundcolor]" />
                  <?php _e("(ex: FF0000)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="form_padding">
                  <?php _e("Form Padding:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="8px" class="ccf-width75" name="object[form_padding]" />
                  <?php _e("(ex: 5px or 1em)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="title_margin">
                  <?php _e("Title Margin:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="5px" class="ccf-width75" name="object[title_margin]" />
                  <?php _e("(ex: 5px or 1em)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="title_margin">
                  <?php _e("Dropdown Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="auto" class="ccf-width75" name="object[dropdown_width]" />
                  <?php _e("(ex: 30px, 20%, or auto)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="success_popover_bordercolor">
                  <?php _e("Success Popover Border Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="efefef" class="ccf-width75 colorfield" name="object[success_popover_bordercolor]" />
                  <?php _e("(ex: FF0000)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="success_popover_fontsize">
                  <?php _e("Success Popover Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="12px" class="ccf-width75" name="object[success_popover_fontsize]" />
                  <?php _e("(ex: 12px, 1em, 100%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="success_popover_height">
                  <?php _e("Success Popover Height:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="200px" class="ccf-width75" name="object[success_popover_height]" />
                  <?php _e("(ex: 200px, 6em, 50%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="field_borderround">
                  <?php _e("Field Border Roundness:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="6px" class="ccf-width75" name="object[field_borderround]" />
                  <?php _e("(ex: 6px, or 0px)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="tooltip_fontsize">
                  ...
                  </label> <input type="text" maxlength="20" value="12px" class="ccf-width75" name="object[tooltip_fontsize]" />
                  <?php _e("(ex: 12px, 1em, 100%)", 'custom-contact-forms'); ?>
                 </li>
                     <li><label for="tooltip_fontcolor">
                  <?php _e("Tooltip Font Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="ffffff" class="ccf-width75 colorfield" name="object[tooltip_fontcolor]" />
                  <?php _e("(ex: ffffff or white)", 'custom-contact-forms'); ?>
                 </li>
                     <li><input name="object_type" type="hidden" value="style" /> <input type="submit" value="<?php _e("Create Style", 'custom-contact-forms'); ?>" name="object_create" class="create-button" /></li>
                  </ul>
               </form>
               <div class="ccf-clear"></div>
            </div>
         ...
         ?>
              <tr class="row-style-<?php echo esc_attr($style->id); ?> <?php if ($i % 2 == 0) echo 'ccf-evenrow'; ?>">
                  ...
                  * <?php _e("Slug:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="30" value="<?php echo esc_attr($style->style_slug); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][style_slug]" /> <br /> <label>
                  <?php _e("Font Family:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="120" value="<?php echo esc_attr($style->form_fontfamily); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][form_fontfamily]" /> <br /> <label>
                  <?php _e("Textarea Background", 'custom-contact-forms'); ?>
                  <br />
                  <?php _e("Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->textarea_backgroundcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][textarea_backgroundcolor]" /> <br /> <label>
                  <?php _e("Success Popover", 'custom-contact-forms'); ?>
                  <br />
                  <?php _e("Border Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->success_popover_bordercolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][success_popover_bordercolor]" /> <br /> <label>
                  <?php _e("Tooltip", 'custom-contact-forms'); ?>
                  <a href="javascript:void(0)" class="toollink" title="<?php _e("A tooltip is the little box that fades in displaying 'Field Instructions' when a user selects a particular field.", 'custom-contact-forms'); ?>">(?)</a>
                  <?php _e("Font Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->tooltip_fontcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][tooltip_fontcolor]" /> <br /> <label>
                  <?php _e("Button Background", 'custom-contact-forms'); ?><br />
                     ...
                     </select> <br /> <input type="button" class="single-save" value="<?php _e('Save', 'custom-contact-forms'); ?>" />
                        <div class="loading-img-container">
                           <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" width="16" height="16" class="ccf-hide loading-img-inner-style-<?php echo esc_attr($style->id); ?>" />
                        </div> <br /> <input type="button" class="single-delete" value="<?php _e('Delete', 'custom-contact-forms'); ?>" /> <input class="object-type" type="hidden" name="objects[<?php echo esc_attr($style->id); ?>][object_type]" value="style" /> <input class="object-id" name="objects[<?php echo esc_attr($style->id); ?>][object_id]" type="hidden" value="<?php echo esc_attr($style->id); ?>" /></td>
                     <td><label>
                  <?php _e("Form Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->form_width); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][form_width]" /> <br /> <label>
                  <?php _e("Text Field Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->input_width); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][input_width]" /> <br /> <label>
                  <?php _e("Textarea Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->textarea_width); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][textarea_width]" /> <br /> <label>
                  <?php _e("Textarea Height:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->textarea_height); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][textarea_height]" /> <br /> <label>
                  <?php _e("Dropdown Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->dropdown_width); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][dropdown_width]" /> <br /> <label>
                  <?php _e("Label Margin:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->label_margin); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][label_margin]" /> <br /> <label>
                  <?php _e("Success Popover", 'custom-contact-forms'); ?>
                  <br />
                  <?php _e("Height:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->success_popover_height); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][success_popover_height]" /> <br /> <label>
                  <?php _e("Button Background:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="200" value="<?php echo esc_attr($style->submit_background); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][submit_background]" /> <br /></td>
                     <td><label>
                  <?php _e("Label Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->label_width); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][label_width]" /> <br /> <label>
                  <?php _e("Button Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->submit_width); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][submit_width]" /> <br /> <label>
                  <?php _e("Button Height:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->submit_height); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][submit_height]" /> <br /> <label>
                  <?php _e("Field Background Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->field_backgroundcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][field_backgroundcolor]" /> <br /> <label>
                  <?php _e("Title Margin:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->title_margin); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][title_margin]" /> <br /> <label>
                  <?php _e("Success Popover", 'custom-contact-forms'); ?>
                  <br />
                  <?php _e("Title Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->success_popover_title_fontsize); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][success_popover_title_fontsize]" /> <label>
                  <?php _e("Form Background Color:", 'custom-contact-forms'); ?>
                  </label> <input type="text" class="colorfield" maxlength="20" value="<?php echo esc_attr($style->form_backgroundcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][form_backgroundcolor]" /></td>
                     <td><label>
                  <?php _e("Title Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->title_fontsize); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][title_fontsize]" /> <br /> <label>
                  <?php _e("Label Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->label_fontsize); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][label_fontsize]" /> <br /> <label>
                  <?php _e("Field Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->field_fontsize); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][field_fontsize]" /> <br /> <label>
                  <?php _e("Button Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->submit_fontsize); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][submit_fontsize]" /> <br /> <label>
                  <?php _e("Form Padding:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->form_padding); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][form_padding]" /> <br /> <label>
                  <?php _e("Success Popover", 'custom-contact-forms'); ?>
                  <br />
                  <?php _e("Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->success_popover_fontsize); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][success_popover_fontsize]" /> <br /> <label>
                  <?php _e("Tooltip", 'custom-contact-forms'); ?>
                  <a href="javascript:void(0)" class="toollink" title="<?php _e("A tooltip is the little box that fades in displaying 'Field Instructions' when a user selects a particular field.", 'custom-contact-forms'); ?>">(?)</a>
                  <?php _e("Background Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->tooltip_backgroundcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][tooltip_backgroundcolor]" /></td>
                     <td><label>
                  <?php _e("Title Font Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->title_fontcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][title_fontcolor]" /> <br /> <label>
                  <?php _e("Label Font Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->label_fontcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][label_fontcolor]" /> <br /> <label>
                  <?php _e("Field Font Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->field_fontcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][field_fontcolor]" /> <br /> <label>
                  <?php _e("Button Font Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->submit_fontcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][submit_fontcolor]" /> <br /> <label>
                  <?php _e("Form Margin:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->form_margin); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][form_margin]" /> <br /> <label>
                  <?php _e("Success Popover", 'custom-contact-forms'); ?>
                  <br />
                  <?php _e("Font Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->success_popover_fontcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][success_popover_fontcolor]" /> <br /> <label>
                  <?php _e("Tooltip Font Size:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->tooltip_fontsize); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][tooltip_fontsize]" /></td>
                     <td><label>
                  <?php _e("Form Border Style:", 'custom-contact-forms'); ?>
                  </label> <select name="objects[<?php echo esc_attr($style->id); ?>][values][form_borderstyle]">
                  ...
                  <?php _e("Form Border Width:", 'custom-contact-forms'); ?>
                  </label> <input type="text" maxlength="20" value="<?php echo esc_attr($style->form_borderwidth); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][form_borderwidth]" /> <br /> <label>
                  <?php _e("Form Border Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->form_bordercolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][form_bordercolor]" /> <br /> <label>
                  <?php _e("Field Border Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->field_bordercolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][field_bordercolor]" /> <br /> <label>
                  <?php _e("Field Border Style:", 'custom-contact-forms'); ?>
                  </label> <select name="objects[<?php echo esc_attr($style->id); ?>][values][field_borderstyle]">
                  ...
                  <br />
                  <?php _e("Title Font Color:", 'custom-contact-forms'); ?>
                  </label> <input class="colorfield" type="text" maxlength="20" value="<?php echo esc_attr($style->success_popover_title_fontcolor); ?>" name="objects[<?php echo esc_attr($style->id); ?>][values][success_popover_title_fontcolor]" /> <br /> <label>
                  <?php _e("Field Border Roundness:", 'custom-contact-forms'); ?>
                  </label> <input name="objects[<?php echo esc_attr($style->id); ?>][values][field_borderround]" value="<?php echo esc_attr($style->field_borderround); ?>" type="text" maxlength="20" /></td>
                  </tr>
              <?php
         $i++;
         }
...
&lt;/form&gt;</textarea>
      ...
      <div class="postbox">
         <h3>Shortcode Daftar Tanya Jawab yang di publish</h3>
         <div class="inside">[custom_qa_publish]</div>
      </div>
      ...
      <ul id="plugin-nav">
         ...
         <div id="create-fields" class="postbox">
            ...
            <div class="inside">
               <form id="ccf-create-field" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
                  ...
                  <ul class="left">
                     <li><label for="field_slug">*
                        <?php _e("Field Slug:", 'custom-contact-forms'); ?>
                        </label> <input class="ccf-width250" name="object[field_slug]" type="text" maxlength="40" /> <br />
                     ...
                     <li><label for="field_label">
                        <?php _e("Field Label:", 'custom-contact-forms'); ?>
                        </label> <input class="ccf-width250" name="object[field_label]" type="text" maxlength="100" /> <br />
                        <?php _e("The field label is displayed next to the field and is visible to the user.", 'custom-contact-forms'); ?>
                       </li>
                     <li><label for="field_type">*
                        <?php _e("Field Type:", 'custom-contact-forms'); ?>
                        </label> <select name="object[field_type]" class="field-type-selector">
                           <option>Text</option>
                           <option>Date</option>
                           <option>File</option>
                     ...
                     <li><label for="field_value">
                        <?php _e("Initial Value:", 'custom-contact-forms'); ?>
                        </label> <input class="ccf-width250" name="object[field_value]" type="text" maxlength="50" /> <br />
                     ...
                     <li><label for="field_maxlength">
                        <?php _e("Max Length:", 'custom-contact-forms'); ?>
                        </label> <input class="ccf-width250" size="10" name="object[field_maxlength]" type="text" maxlength="4" /> <br />
                        <?php _e("0 for no limit; only applies to Text fields", 'custom-contact-forms'); ?>
                       </li>
                  </ul>
                  <ul class="right">
                     <li><label for="field_required">*
                        <?php _e("Required Field:", 'custom-contact-forms'); ?>
                        </label> <select name="object[field_required]">
                           <option value="0">
                     ...
                     <li><label for="field_instructions">
                        <?php _e("Field Instructions:", 'custom-contact-forms'); ?>
                        </label> <input class="ccf-width250" name="object[field_instructions]" type="text" /> <br />
                     ...
                     <li><label for="field_class">
                        <?php _e("Field Class:", 'custom-contact-forms'); ?>
                        </label> <input class="ccf-width250" name="object[field_class]" type="text" /> <br />
                     ...
                     <li><label for="field_error">
                        <?php _e("Field Error:", 'custom-contact-forms'); ?>
                        </label> <input class="ccf-width250" name="object[field_error]" type="text" /> <br />
                     ...
                     <li class="file-fields"><label for="field_max_upload_size">
                        <?php _e("Max File Size Allowed:", 'custom-contact-forms'); ?>
                        </label> <input class="ccf-width250" name="object[field_max_upload_size]" value="5000" type="text" /> <?php _e('KB', 'custom-contact-forms'); ?>
                        <br />
                     ...
                     <li class="file-fields"><label for="field_allowed_file_extensions">
                        <?php _e("Allowed File Extensions for Upload:", 'custom-contact-forms'); ?>
                        </label> <input class="ccf-width250" name="object[field_allowed_file_extensions]" type="text" /> <br />
                     ...
                     <li><input type="hidden" name="object[user_field]" value="1" /> <input type="hidden" name="object_type" value="field" /> <input type="submit" value="<?php _e("Create Field", 'custom-contact-forms'); ?>" name="object_create" class="create-button" /></li>
         ...
         <form class="ccf-edit-ajax" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
            ...
            <table class="widefat post" id="manage-fields" cellspacing="0">
               ...
               //if ($fields[$i]->user_field) { $z--; continue; }
               ...
               ?>
                  <tr class="row-field-<?php echo esc_attr($fields[$i]->id); ?> <?php if ($z % 2 == 1) echo ' ccf-evenrow'; ?>">
                     ...
                     <td><input type="hidden" class="object-type" name="objects[<?php echo esc_attr($fields[$i]->id); ?>][object_type]" value="field" /> <input type="hidden" class="object-id" name="objects[<?php echo esc_attr($fields[$i]->id); ?>][object_id]" value="<?php echo esc_attr($fields[$i]->id); ?>" /> <input type="button" class="single-save" value="<?php _e('Save', 'custom-contact-forms'); ?>" /> <input type="button" class="single-delete" value="<?php _e('Delete', 'custom-contact-forms'); ?>" /> <input type="button" class="fields-options-expand-link" value="<?php _e('Options', 'custom-contact-forms'); ?>">
                        <div class="loading-img-container">
                           <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" width="16" height="16" class="ccf-hide loading-img-inner-field-<?php echo esc_attr($fields[$i]->id); ?>" />
                        </div></td>
                  </tr>
                    ...
                    <tr class="row-field-<?php echo esc_attr($fields[$i]->id); ?> <?php if ($z % 2 == 1) echo 'ccf-evenrow'; ?>">
                     <td class="fields-extra-options ccf-hide" colspan="8">
                        <div class="one">
                           <label for="field_instructions"> <a href="javascript:void(0)" class="toollink" title="<?php _e('If this is filled out, a tooltip popover displaying this text will show when the field is selected.', 'custom-contact-forms'); ?>">(?)</a>
                        <?php _e("Field Instructions:", 'custom-contact-forms'); ?>
                        </label>
                        ...
                        <div class="two">
                           <label for="field_error"> <a href="javascript:void(0)" class="toollink" title="<?php _e('This lets you customize the error message displayed when this field is required and left blank.', 'custom-contact-forms'); ?>">(?)</a>
                         <?php _e("Field Error:", 'custom-contact-forms'); ?>
                        </label>
                        ...
                        <div class="three">
                          ...
                          <?php _e('Max Length:', 'custom-contact-forms'); ?>
                              </label> <input type="text" class="ccf-width75" name="objects[<?php echo esc_attr($fields[$i]->id); ?>][values][field_maxlength]" value="<?php echo esc_attr($fields[$i]->field_maxlength); ?>" /> <br />
                        <?php } ?>
                         ...
                         <?php _e("Max Upload Size:", 'custom-contact-forms'); ?></label> <input type="text" class="ccf-width75" name="objects[<?php echo esc_attr($fields[$i]->id); ?>][values][field_max_upload_size]" value="<?php echo esc_attr($fields[$i]->field_max_upload_size); ?>" /><?php _e('KB', 'custom-contact-forms'); ?>
                              <br /> <label for="field_allowed_file_extensions"><a href="javascript:void(0)" class="toollink" title="<?php _e('If a user tries to upload a file with an extension not in this list, an error will be shown. If this is left blank, then all file extensions will be accepted. Separate file extensions with a comma. Ex: doc, jpg, jpeg, bmp, gif, txt', 'custom-contact-forms'); ?>">(?)</a>
                         <?php _e("Allowed File Extensions:", 'custom-contact-forms'); ?></label> <input type="text" class="ccf-width75" name="objects[<?php echo esc_attr($fields[$i]->id); ?>][values][field_allowed_file_extensions]" value="<?php $exts = unserialize($fields[$i]->field_allowed_file_extensions); echo (!empty($exts)) ? esc_attr(@implode(', ', $exts)) : ''; ?>" />
                            <?php } ?>
                            </div>
                       <?php
            ...
            echo esc_html($this_option->option_slug);?></li>
                     ...
                     echo '</ul>';
                 ?>
                                    <input class="attached-update-button" type="button" value="<?php _e('Save Option Configuration', 'custom-contact-forms'); ?>" /> <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" width="16" height="16" class="ccf-hide loading-img-field-config-field-<?php echo esc_attr($fields[$i]->id); ?>" />
               ...
               <form class="ccf-edit-ajax" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
                  ...
                  $option_count = count($options);
                      ...
                      ?>
                           ...
                           <td><select name="objects[<?php echo esc_attr($option->id); ?>][values][option_dead]"><option value="0"><?php _e('No', 'custom-contact-forms'); ?></option>
                                 <option <?php if ($option->option_dead == 1) echo 'selected="selected"'; ?> value="1"><?php _e('Yes', 'custom-contact-forms'); ?></option></select></td>
                           <td><input type="hidden" class="object-type" name="objects[<?php echo esc_attr($option->id); ?>][object_type]" value="field_option" /> <input type="hidden" class="object-id" name="objects[<?php echo esc_attr($option->id); ?>][object_id]" value="<?php echo esc_attr($option->id); ?>" /> <input type="button" class="single-save" value="<?php _e('Save', 'custom-contact-forms'); ?>" /> <input type="button" class="single-delete" value="<?php _e('Delete', 'custom-contact-forms'); ?>" />
                              <div class="loading-img-container">
                                 <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" width="16" height="16" class="ccf-hide loading-img-inner-field_option-<?php echo esc_attr($option->id); ?>" />
                              </div></td>
                        </tr>
                              <?php
                      $i++;
                  ...
                  <select class="bulk-dropdown" name="object_bulk_action">
                     ...
                     <option value="edit"><?php _e('Save', 'custom-contact-forms'); ?></option>
                     <option value="delete"><?php _e('Delete', 'custom-contact-forms'); ?></option>
                  </select> <input type="submit" class="bulk-apply" name="object_bulk_apply" value="<?php _e('Apply', 'custom-contact-forms'); ?>" /> <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" width="16" height="16" class="loading-img" />
               </form>
            </div>
         </div>
         ...
         <div id="create-field-options" class="postbox">
            ...
            <div class="inside">
               <form id="ccf-create-field-option" method="post" action="<?php echo $_SERVER['REQUEST_URI']?>">
                  <input type="hidden" name="selected_tab" value="field-options" />
                  <ul>
                     <li><label for="object[option_slug]">*
                                  <?php _e("Option Slug:", 'custom-contact-forms'); ?>
                                  </label> <input class="ccf-width250" maxlength="20" type="text" name="object[option_slug]" /> <br />
                     ...
                     <li><label for="object[option_label]">*
                                  <?php _e("Option Label:", 'custom-contact-forms'); ?>
                                  </label> <input class="ccf-width250" type="text" name="object[option_label]" /> <br />
                                  <?php _e("This is what is shown to the user in the dropdown or radio field. Example: 'United States'", 'custom-contact-forms'); ?>
                                </li>
                     <li><label for="object[option_value]">
                                  <?php _e("Option Value:", 'custom-contact-forms'); ?>
                                  </label> <input class="ccf-width250" type="text" name="object[option_value]" /> <br />
                     ...
                     <li><label for="object[option_dead]"><?php _e("Is Dead Option:", 'custom-contact-forms'); ?></label> <select name="object[option_dead]"><option value="0"><?php _e('No', 'custom-contact-forms'); ?></option>
                           <option value="1"><?php _e('Yes', 'custom-contact-forms'); ?></option></select> <br />
                     ...
                     <li><input name="object_type" type="hidden" value="field_option" /> <input type="submit" class="object-action" name="object_create" value="<?php _e("Create Field Option", 'custom-contact-forms'); ?>" /></li>
                  </ul>
               </form>
            </div>
         </div>
   ...
   <form class="ccf-edit-ajax" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
      <table class="widefat post" id="form-submissions-table" cellspacing="0">
         ...
         ?>
               ...
               <td class="ccf-alignright"><input type="button" class="submission-content-expand-button" value="<?php _e('Expand', 'custom-contact-forms'); ?>" /> <input type="button" class="single-delete" value="<?php _e('Delete', 'custom-contact-forms'); ?>" /> <input class="object-id" type="hidden" name="objects[<?php echo esc_attr($data_object->id); ?>][object_id]" value="<?php echo esc_attr($data_object->id); ?>" /> <input type="hidden" class="object-type" name="objects[<?php echo esc_attr($data_object->id); ?>][object_type]" value="form_submission" />
                  <div class="loading-img-container">
                     <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" width="16" height="16" class="ccf-hide loading-img-inner-form_submission-<?php echo esc_attr($data_object->id); ?>" />
                  </div></td>
            </tr>
      ...
      <select class="bulk-dropdown" name="object_bulk_action">
         <option value="0"><?php _e('Bulk Actions', 'custom-contact-forms'); ?></option>
         <option value="delete"><?php _e('Delete', 'custom-contact-forms'); ?></option>
      </select> <input type="submit" class="bulk-apply" name="object_bulk_apply" value="<?php _e('Apply', 'custom-contact-forms'); ?>" /> <img src="<?php echo plugins_url(); ?>/kip/images/wpspin_light.gif" class="loading-img" width="16" height="16" />
   ...
   <div class="plugin-header">
      ...
      <div class="inside">
         <form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
            <ul class="gleft">
               <li><label for="email_form_submissions">
                  <?php _e("Email Form Submissions:", 'custom-contact-forms'); ?>
                  </label> <select name="settings[email_form_submissions]">
                     <option value="1">
                    <?php _e("Yes", 'custom-contact-forms'); ?>
                    </option>
               ...
               <li><label for="default_to_email">
                  <?php _e("Default Email:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[default_to_email]" value="<?php echo esc_attr($admin_options['default_to_email']); ?>" type="text" maxlength="100" /></li>
               <li class="descrip">
               ...
               <li><label for="enable_jquery">
                  <?php _e("Front End JQuery:", 'custom-contact-forms'); ?>
                  </label> <select name="settings[enable_jquery]">
                     <option value="1">
               ...
               <li><label for="default_from_email">
                  <?php _e("Default From Email:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[default_from_email]" value="<?php echo esc_attr($admin_options['default_from_email']); ?>" type="text" maxlength="100" /></li>
               <li class="descrip">
               ...
               <li><label for="default_from_name">
                  <?php _e("Default From Name:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[default_from_name]" value="<?php echo esc_attr($admin_options['default_from_name']); ?>" type="text" maxlength="100" /></li>
               <li class="descrip">
               ...
               <li><label for="default_form_subject">
                  <?php _e("Default Email Subject:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[default_form_subject]" value="<?php echo esc_attr($admin_options['default_form_subject']); ?>" type="text" /></li>
               <li class="descrip">
                  <?php _e("Default subject to be included in all form emails.", 'custom-contact-forms'); ?>
                 </li>
               <li><label for="enable_dashboard_widget">
                  <?php _e("Enable Dashboard Widget:", 'custom-contact-forms'); ?>
                  </label> <select name="settings[enable_dashboard_widget]">
                     <option value="1">
                    <?php _e("Enabled", 'custom-contact-forms'); ?>
                    </option>
               ...
               <li><label for="dashboard_access">
                  <?php _e("Dashboard Widget Accessibility:", 'custom-contact-forms'); ?>
                  </label> <select name="settings[dashboard_access]">
                     <option value="2">
               ...
               <li><label for="code_type">
                  <?php _e("Use Code Type:", 'custom-contact-forms'); ?>
                  </label> <select name="settings[code_type]">
                     <option>XHTML</option>
               ...
               <li><label for="recaptcha_public_key">
                  <?php _e("reCaptcha Public Key:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[recaptcha_public_key]" value="<?php echo esc_attr( $admin_options['recaptcha_public_key'] ); ?>" type="text" /></li>
               <li class="descrip">
               ...
               <li><label for="recaptcha_private_key">
                  <?php _e("reCaptcha Private Key:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[recaptcha_private_key]" value="<?php echo esc_attr( $admin_options['recaptcha_private_key'] ); ?>" type="text" /></li>
               <li class="descrip">
                  <?php _e( 'This key comes from <a href="http://google.com/recaptcha">reCaptcha</a> and allows you to use the recaptcha fixed field.', 'custom-contact-forms' ); ?>
                 </li>
            ...
            <ul class="gright">
               <li><label for="form_success_message_title">
                  <?php _e("Default Form Success Message Title:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[form_success_message_title]" value="<?php echo esc_attr($admin_options['form_success_message_title']); ?>" type="text" /></li>
               <li class="descrip">
               ...
               <li><label for="form_success_message">
                  <?php _e("Default Form Success Message:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[form_success_message]" value="<?php echo esc_attr($admin_options['form_success_message']); ?>" type="text" /></li>
               <li class="descrip">
               ...
               <li><label for="default_form_error_header">
                  <?php _e("Default Form Error Header:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[default_form_error_header]" value="<?php echo esc_attr($admin_options['default_form_error_header']); ?>" type="text" /></li>
               <li class="descrip">
               ...
               <li><label for="remember_field_values">
                  <?php _e("Remember Field Values:", 'custom-contact-forms'); ?>
                  </label> <select name="settings[remember_field_values]">
                     <option value="1">
                    <?php _e("Yes", 'custom-contact-forms'); ?>
                    </option>
               ...
               <li><label for="enable_widget_tooltips">
                  <?php _e("Tooltips in Widget:", 'custom-contact-forms'); ?>
                  </label> <select name="settings[enable_widget_tooltips]">
                     <option value="1">
                    <?php _e("Enabled", 'custom-contact-forms'); ?>
                    </option>
               ...
               <li><label for="form_page_inclusion_only">
                  <?php _e("Restrict Frontend JS and CSS to Form Pages Only:", 'custom-contact-forms'); ?>
                  </label> <select name="settings[form_page_inclusion_only]">
                     <option value="1">
                    <?php _e("Yes", 'custom-contact-forms'); ?>
                    </option>
               ...
               <li><label for="default_form_bad_permissions">
                  <?php _e("Default Permissions Error:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[default_form_bad_permissions]" value="<?php echo esc_attr($admin_options['default_form_bad_permissions']); ?>" type="text" /></li>
               <li class="descrip">
               ...
               <li><label for="enable_form_access_manager">
                  <?php _e("Form Access Capabilities:", 'custom-contact-forms'); ?>
                  </label> <select name="settings[enable_form_access_manager]">
                     <option value="0">
                    <?php _e("Disabled", 'custom-contact-forms'); ?>
                    </option>
               ...
               <li><label for="max_file_upload_size">
                  <?php _e("Maximum File Upload Size:", 'custom-contact-forms'); ?>
                  </label> <input name="settings[max_file_upload_size]" class="ccf-width75" type="text" value="<?php echo esc_attr($admin_options['max_file_upload_size']); ?>" /> <?php _e("MB"); ?>
               ...
               <li><label> <input value="1" type="checkbox" name="settings[show_widget_home]" <?php if ($admin_options['show_widget_home'] == 1) echo 'checked="checked"'; ?> />
                  <?php _e("On Homepage", 'custom-contact-forms'); ?>
                  </label> <label> <input value="1" type="checkbox" name="settings[show_widget_pages]" <?php if ($admin_options['show_widget_pages'] == 1) echo 'checked="checked"'; ?> />
                  <?php _e("On Pages", 'custom-contact-forms'); ?>
                  </label> <label> <input value="1" type="checkbox" name="settings[show_widget_singles]" <?php if ($admin_options['show_widget_singles'] == 1) echo 'checked="checked"'; ?> />
                  <?php _e("On Single Posts", 'custom-contact-forms'); ?>
                  </label> <br /> <label> <input value="1" type="checkbox" name="settings[show_widget_categories]" <?php if ($admin_options['show_widget_categories'] == 1) echo 'checked="checked"'; ?> />
                  <?php _e("On Categories", 'custom-contact-forms'); ?>
                  </label> <label> <input value="1" type="checkbox" name="settings[show_widget_archives]" <?php if ($admin_options['show_widget_archives'] == 1) echo 'checked="checked"'; ?> />
                  <?php _e("On Archives", 'custom-contact-forms'); ?>
                  </label></li>

               <li><input type="submit" value="<?php _e("Update", 'custom-contact-forms'); ?>" name="general_settings" /></li>
            </ul>
         </form>
      </div>
   </div>
   <div id="configure-mail" class="postbox">
      ...
      <div class="inside">
         <form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
            ...
            <label for="mail_function"><?php _e("* Send My Emails Using the Following:", 'custom-contact-forms'); ?></label> <select name="mail_config[mail_function]">
               ...
               <ul class="right">
                  <li><label for="smtp_authentication"><?php _e("SMTP Authentication:", 'custom-contact-forms'); ?></label> <select name="mail_config[smtp_authentication]"><option value="0"><?php _e("None Needed", 'custom-contact-forms'); ?></option>
                        <option <?php if ($admin_options['smtp_authentication'] == 1) echo 'selected="selected"'; ?> value="1"><?php _e("Use SMTP Username/Password", 'custom-contact-forms'); ?></option></select></li>
   ...
   <div id="import" class="postbox">
      ...
      <div class="inside">
         <form method="post" enctype="multipart/form-data" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
            ...
            <ul>
               <li><input type="checkbox" checked="checked" name="ccf_import_overwrite_settings" value="1" /> <label for="ccf_import_overwrite_settings">
                  <?php _e("Use General Settings", 'custom-contact-forms'); ?>
                  </label></li>
               <li><input type="checkbox" checked="checked" name="ccf_import_forms" value="1" /> <label for="ccf_import_forms">
                  <?php _e("Forms", 'custom-contact-forms'); ?>
                  </label></li>
               <li><input type="checkbox" checked="checked" name="ccf_import_saved_submissions" value="1" /> <label for="ccf_import_saved_submissions">
                  <?php _e("Form Submissions", 'custom-contact-forms'); ?>
                  </label></li>
               <li><input type="checkbox" checked="checked" name="ccf_import_fields" value="1" /> <label for="ccf_import_fields">
                  <?php _e("Fields", 'custom-contact-forms'); ?>
                  </label></li>
               <li><input type="checkbox" checked="checked" name="ccf_import_forms" value="1" /> <label for="ccf_import_forms">
                  <?php _e("Forms", 'custom-contact-forms'); ?>
                  </label></li>
               <li><input type="checkbox" checked="checked" name="ccf_import_field_options" value="1" /> <label for="ccf_import_field_options">
                  <?php _e("Field Options", 'custom-contact-forms'); ?>
                  </label></li>
               <li><input type="checkbox" checked="checked" name="ccf_import_styles" value="1" /> <label for="ccf_import_styles">
            ...
            <p class="choose_import">
               <label for="import_file">
                 ...
                 </label> <input type="file" name="import_file" />
            </p>
            <input name="ccf_clear_import" type="submit" value="<?php _e("Clear and Import", 'custom-contact-forms'); ?>" /> <input type="checkbox" name="ccf_import_confirm" value="1" />
...
<?php
                  ...
                  $_POST['mail_config'] = array_map(array('ccf_utils', 'encodeOption'), $_POST['mail_config']);
                  $admin_options = array_merge($admin_options, $_POST['mail_config']);
                  $this->action_complete = __('Pengaturan Email Tersimpan', 'custom-contact-forms');
                  update_option(parent::getAdminOptionsName(), $admin_options);
               }
               ccf_utils::load_module('export/custom-contact-forms-export.php');
               ?>
<div id="customcontactforms-admin">
   <div class="">
      <h2>
                     <?php _e("KIP", 'custom-contact-forms'); ?>
                  </h2>
   </div>
      ...
      <h3 class="hndle">
         <span>
                    <?php _e("Pengaturan Umum", 'custom-contact-forms'); ?>
                    </span>
      </h3>
      <div class="inside">
         <form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
            <ul class="gleft">
               <li><label for="email_form_submissions">
                        <?php _e("Aktifkan Notifikasi Email:", 'custom-contact-forms'); ?>
                        </label> <select name="settings[email_form_submissions]">
                     <option value="1">
                          <?php _e("Yes", 'custom-contact-forms'); ?>
                          </option>
               ...
               <li class="descrip">
                        <?php _e("Notifikasi via email ke Pejabat PID bila ada user baru terdaftar<br/>
Notifikasi via email ke Pejabat PID bila ada pertanyaan yg sudah lewat dari 7 hari belum terjawab<br/>
Notifikasi via email ke Pemohon Informasi bila pertanyaannya dijawab/ditolak", 'custom-contact-forms'); ?>
                       </li>
            </ul>
            <ul class="gright">
               <br />
...
<?php
      ...
      public function pendingUsersPage() {
         $this->handleAdminPostRequests();
         ccf_utils::load_module('export/custom-contact-forms-user-data.php');
         $user_data_array = parent::selectAllUserData(FORM_ID_REGISTERED);
         $formField = parent::getAttachedFieldsArray(FORM_ID_REGISTERED);
         ?>
<h3 class="hndle">
   <span>
              <?php _e("Daftar Pengguna Pending", 'custom-contact-forms'); ?>
              </span>
</h3>
<form class="ccf-edit-ajax" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
   <table class="widefat post" id="form-submissions-table" cellspacing="0">
      <thead>
         <tr>
            <th scope="col" class="manage-column ccf-width250"><?php _e("Waktu Pendaftaran", 'custom-contact-forms'); ?></th>
                    <?php
                    foreach ($formField as $fieldId) {
                       $field = parent::selectField($fieldId);
                       ?>
                          <?php if ($field->field_slug == 'uploadpengenal') {?>
                          <th scope="col" class="manage-column ccf-width250">Berkas Tanda Pengenal</th>
                          <?php } else {?>
                          <th scope="col" class="manage-column ccf-width250"><?php echo esc_html($field->field_label); ?></th>
                          <?php }?>
                       <?php
                    }
                     ?>
                    <th scope="col" class="manage-column ccf-width250"><?php _e("Aksi", 'custom-contact-forms'); ?></th>

         </tr>
      </thead>

      <tbody>
      <?php if (!$user_data_array) {?>
            <tr><td colspan="30">Tidak ada pengguna pending</td></tr>
         <?php }?>
                 <?php foreach ($user_data_array as $data_object) {
                    $data = new CustomContactFormsUserData(array('form_id' => $data_object->data_formid, 'data_time' => $data_object->data_time, 'form_page' => $data_object->data_formpage, 'encoded_data' => $data_object->data_value));
                 ?>
                 <tr>
            <td><?php echo esc_html(date('F d, Y h:i:s A', $data->getDataTime())); ?></td>
                    <?php
                    $data_array = $data->getDataArray();
                    foreach ($data_array as $item_value) {
                       ?>
                       <td>
                          <?php if (strpos($item_value, "[file link") !== false) {
                     if (strpos($data->parseUserData($item_value), ".jpg") !== false ||
                           strpos($data->parseUserData($item_value), ".jpeg") !== false ||
                           strpos($data->parseUserData($item_value), ".png") !== false ||
                           strpos($data->parseUserData($item_value), ".gif") !== false ||
                           strpos($data->parseUserData($item_value), ".bmp") !== false ||
                           strpos($data->parseUserData($item_value), ".tif") !== false
                        ) { ?>
                        <a class="fancybox" href="<?php echo $data->parseUserData($item_value, true) ?>">View Gambar</a><br />
                    <?php }
                  } else {
                   ?>
                  <?php echo $data->parseUserData($item_value); }?>
                       </td>
                       <?php
                    }
                    ?>
                    <td>
               <form>
                  <input type="hidden" value="<?php echo $data_object->id ?>" name="object_id" /> <input type="submit" class="button single-approve" value="Setujui" />
               </form>
            </td>
         </tr>
                 <?php } ?>
...
public function pendingUsersPageWidget() {
   $this->handleAdminPostRequests();
   ccf_utils::load_module('export/custom-contact-forms-user-data.php');
   $user_data_array = parent::selectAllUserData(FORM_ID_REGISTERED);
   $formField = parent::getAttachedFieldsArray(FORM_ID_REGISTERED);
...
<img width="50" height="50" class="avatar avatar-50 photo avatar-default" src="http://0.gravatar.com/avatar/ad516503a11cd5ca435acc9bb6523536?s=50" alt="">
         <div class="dashboard-comment-wrap">
         <h4 class="comment-meta">
         <cite class="comment-author"><?php echo $data->parseUserData($data_array['namalengkap']); ?></cite>
         </h4>


<ul class="subsubsub" style="margin-top:0px;margin-bottom:0px">
   <li class="detail"><a href="<?php echo admin_url() . 'admin.php?page=ccf-pending'?>">Lihat Detail</a></li>
</ul>
...
<?php }
   ...
   public function pendingQuestionsPageWidget() {
      $this->handleAdminPostRequests();
ccf_utils::load_module('export/custom-contact-forms-user-data.php');
$user_data_array = parent::selectAllUserData(FORM_ID_QUESTION, 5);
$formField = parent::getAttachedFieldsArray(FORM_ID_QUESTION);
?>
<div data-wp-lists="list:comment" id="the-comment-list">
<?php if (!$user_data_array) {?>
Tidak ada pertanyaan pending
<?php }?>
   <?php foreach ($user_data_array as $data_object) {
      $data = new CustomContactFormsUserData(array('form_id' => $data_object->data_formid, 'data_time' => $data_object->data_time, 'form_page' => $data_object->data_formpage, 'encoded_data' => $data_object->data_value));
      ?>
       <?php
         ...
         $a = new DateTime('@' . $data->getDataTime());
         $b = new DateTime('@' . time());
         $interval = $b->diff($a);
...
<img width="50" height="50" class="avatar avatar-50 photo avatar-default" src="http://0.gravatar.com/avatar/ad516503a11cd5ca435acc9bb6523536?s=50" alt="">
         <div class="dashboard-comment-wrap">
         <h4 class="comment-meta">
         <cite class="comment-author"><?php echo $data->parseUserData($data_array['question']); ?></cite> <br/>
         <cite class="comment-author" style="font-size:11px">Belum terjawab : <span style="color: red"><?php echo $interval->format('%a hari')."\n";
         echo $interval->format('%h jam %i menit')."\n"; ?></span></cite>
         </h4>


<ul class="subsubsub" style="margin-top:0px;margin-bottom:0px">
   <li class="detail"><a href="<?php echo admin_url() . 'admin.php?page=ccf-pending-question'?>">Lihat Detail</a></li>
</ul>
...
<?php }
      ...
      public function pendingQuestionsPage() {
         $this->handleAdminPostRequests();
         ccf_utils::load_module('export/custom-contact-forms-user-data.php');

         $user_data_array = parent::selectAllUserData(FORM_ID_QUESTION);
...
<h3 class="hndle">
   <span>
              <?php _e("Pertanyaan Belum Terjawab", 'custom-contact-forms'); ?>
              </span>
</h3>
<form class="ccf-edit-ajax" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
   <table class="widefat post" id="form-submissions-table" cellspacing="0">
      <thead>
      <?php $labels = array(); foreach ($formField as $fieldId) {

         $field = parent::selectField($fieldId);
            $labels [] = $field->field_slug == 'user_id' ? 'Username' : esc_html($field->field_label);
      }?>
      </thead>
      ...
      <?php
         $the = 0; foreach ($user_data_array as $data_object) {

               $data = new CustomContactFormsUserData(array('form_id' => $data_object->data_formid, 'data_time' => $data_object->data_time, 'form_page' => $data_object->data_formpage, 'encoded_data' => $data_object->data_value));
                $data_array = $data->getDataArray();

            if (is_super_admin() || current_user_can('publish_pages') || (!is_super_admin() && $data_array['user_id'] == get_current_user_id()) ) {
            $the++;
            ?>

      <tr class="inline-edit-row inline-edit-row-post inline-edit-post quick-edit-row quick-edit-row-post inline-edit-post alternate inline-editor">
         <td>
         <fieldset class="inline-edit-col-left" style="width:20%"><div class="inline-edit-col">

            <b>Waktu Bertanya</b> : <br/> <?php echo esc_html(date('F d, Y h:i:s A', $data->getDataTime())); ?><br/><br/>
            <b>Belum terjawab selama</b> : <br/><b style="color:red">
            <?php
            $a = new DateTime('@' . $data->getDataTime());
            $b = new DateTime('@' . time());
            $interval = $b->diff($a);
            ...
            <fieldset class="inline-edit-col-center" style="width:30%"><div class="inline-edit-col">
            <?php $i = 0; foreach ($data_array as $item_key => $item_value) { ?>
            <b><?php echo $labels[$i]; $i++?></b> :

               <?php
                  if ($item_key == "user_id") {
                     $userId = $data->parseUserData($item_value);
                    ?>
                    <a href="javascript:;" class="fix-user-show" id="show-<?php echo $userId; ?>">
            ...
            <?php if (is_super_admin() || current_user_can('publish_pages')) { ?>
            <fieldset class="inline-edit-col-center" style="width:30%"><div class="inline-edit-col">
               <form>
               <?php
               if (!isset($data_array['source'])) {
                  $data_array['source'] = 'Lihat di Web';
               }
               ?>
                  <input type="hidden" value="<?php echo $data_object->id ?>" name="object_id" />
                  <textarea name="answer_text" rows="10" cols="40">Jawaban .... </textarea>
               </form>
               </div>
         </fieldset>
                <fieldset class="inline-edit-col-right" style="width:20%"><div class="inline-edit-col">
               <form>
                  <input type="hidden" value="<?php echo $data_object->id ?>" name="object_id" />
                  <p>
                     <input type="submit"class="button save-q"value="Simpan"/>
                  </p>
                  <p>
                     <input type="submit"class="button publish-q"value="Simpan dan terbitkan"/>
                  </p>
                  <p>
                     <input type="submit"class="button delete-q"value="Tolak" />
                  </p>
               </form>
            </div>
         </fieldset>
      ...
      <?php } ?>
         ...
         <?php } ?>
               ...
               <?php if (!$the) {?>
            <tr><td colspan="30">Tidak ada pertanyaan yang belum terjawab</td></tr>
         <?php }?>
...
<?php
      ...
      public function answeredQuestionsPage() {
         $this->handleAdminPostRequests();
         ccf_utils::load_module('export/custom-contact-forms-user-data.php');
         if (is_super_admin() || current_user_can('publish_pages'))
            $user_data_array = parent::selectAllQuestions();
         else
            $user_data_array = parent::selectAllQuestionsByUser(get_current_user_id());

         $formField = parent::getAttachedFieldsArray(FORM_ID_QUESTION);
         ?>
<h3 class="hndle">
   <span>
              <?php _e("Pertanyaan Terjawab", 'custom-contact-forms'); ?>
              </span>
</h3>
<form class="ccf-edit-ajax" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
   <table class="widefat post" id="form-submissions-table" cellspacing="0">
      ...
      <?php
         foreach ($user_data_array as $data_object) {
               ...
               $data = new CustomContactFormsUserData(array('form_id' => $data_object->data_formid, 'data_time' => $data_object->data_time, 'form_page' => $data_object->data_formpage, 'encoded_data' => $data_object->question_data));
                $data_array = $data->getDataArray();
      ...
      <tr class="inline-edit-row inline-edit-row-post inline-edit-post quick-edit-row quick-edit-row-post inline-edit-post alternate inline-editor">
         ...
         <fieldset class="inline-edit-col-left" style="width:20%"><div class="inline-edit-col">
            ...
            <b>Waktu Bertanya</b> : <br/> <?php echo esc_html(date('d F, Y H:i:s', $data->getDataTime())); ?><br/><br/>
            <b>Dijawab Pada</b> : <br/> <?php echo esc_html(date('d F, Y H:i:s', $data_object->action_time)); ?><br/><br/>
            ...
            <?php if (is_super_admin() || current_user_can('publish_pages')) { ?>
               ...
               <b><?php echo ($data_object->is_rejected  ? "Ditolak" : ($data_object->is_publish ? "Diterbitkan" : "Dijawab namun tidak diterbitkan")) ?></b>
               <?php
               if (!isset($data_array['source'])) {
                  $data_array['source'] = 'Lihat di Web';
               }
               ?>
                  <input type="hidden" value="<?php echo $data_object->id ?>" name="object_id" />
                  <textarea name="answer_text" rows="10" cols="40"><?php echo $data_object->answer_data?></textarea>
               </form>
               ...
               <form>
                  <input type="hidden" value="<?php echo $data_object->id ?>" name="object_id" />
                     <?php if ($data_object->is_rejected) {?>
                        <p>
                     <input type="submit" class="button topending-a" value="Jadikan Pending" />
                  </p>
                     <?php } elseif($data_object->is_publish) {?>
                        <p>
                     <input type="submit" class="button updateans-a" value="Perbarui Jawaban" />
                  </p>
                  <p>
                     <input type="submit" class="button unpublished-a" value="Batal Terbit" />
                  </p>
                     <?php } else {?>
                        <p>
                     <input type="submit" class="button updateans-a" value="Perbarui Jawaban" />
                  ...
                  <p>
                     <input type="submit" class="button publish-a" value="Terbitkan" />
                  </p>
                     <?php }?>
                  </form>
         </div>
         ...
         <?php } else {?>
          ...
          <fieldset class="inline-edit-col-right" style="width:40%"><div class="inline-edit-col">

         <b> <?php echo ($data_object->is_rejected  ? "Ditolak" : ($data_object->is_publish ? "Diterbitkan" : "Dijawab namun tidak diterbitkan")) ?> </b><br/>
            <b>Jawaban : </b> <?php echo $data_object->answer_data?>
         ...
         <?php if (!$user_data_array) {?>
            <tr><td colspan="30">Tidak ada pertanyaan yang terjawab</td></tr>
         <?php }?>
...
<h3 class="hndle">
   <span>
              <?php _e("Daftar Pengguna", 'custom-contact-forms'); ?>
              </span>
</h3>
<table class="widefat post" id="form-submissions-table" cellspacing="0">
   <thead>
      <tr>
         <th scope="col" class="manage-column ccf-width250"><?php _e("Username", 'custom-contact-forms'); ?></th>
         <th scope="col" class="manage-column ccf-width250"><?php _e("Name", 'custom-contact-forms'); ?></th>
         <th scope="col" class="manage-column ccf-width250"><?php _e("Email", 'custom-contact-forms'); ?></th>
         <th scope="col" class="manage-column ccf-width250"><?php _e("Aksi", 'custom-contact-forms'); ?></th>
      </tr>
   </thead>

   <tbody>
         <?php $this->list_authors(1, false);?>
         </tbody>
</table>
...
<h3 class="hndle">
   <span>
                    <?php _e("Daftar Pejabat PID", 'custom-contact-forms'); ?>
                    </span>
</h3>
<table class="widefat post" id="form-submissions-table" cellspacing="0">
   <thead>
...
<?php
      ...
      function list_authors($userlevel = 'all', $show_fullname = true) {
         global $wpdb;
         ...
         */
         ...
         if ($userlevel == 1) {
            ...
            $user_query = new WP_User_Query( array( 'role' => 'applicant', 'fields' => $user_fields ) );
         } elseif ($userlevel == 2) {
            $user_fields = array( 'ID', 'user_login', 'user_nicename', 'user_email', 'user_url' );
            $user_query = new WP_User_Query( array( 'role' => 'ppid', 'fields' => $user_fields ) );
         }
...
//             $authors = $wpdb->get_results("SELECT * from $wpdb->usermeta WHERE meta_value = '$userlevel'");
            ...
            $link  = '<td><a href="javascript:;" id="show-' .$author->ID . '" class="fix-user-show">' . $author->user_login . '</a></td>';
            $link .= '<td>' . $author->display_name . '</td>';
            $link .= '<td>' . $author->user_email . '</td>';
            if ($userlevel == 1) {
               $link .= '<td><form>
                  <input type="hidden" name="user_id" value="' . $author->ID .'">
                     <input type="submit" value="Set sbg Pejabat PID" class="button js-set-ppid">
               </form></td>';
            } else {
               $link .= '<td><form>
                  <input type="hidden" name="user_id" value="' . $author->ID .'">
                     <input type="submit" value="Set sbg Pemohon" class="button js-set-sub">
               </form></td>';
            }
            echo $link;
...
<tr />
               ...
               echo '<tr><td colspan="30">Tidak ada pengguna ' . ($userlevel == 1 ? 'aktif' : 'pejabat PID') . '</td></tr>';
            }
      ...
      function printMyData() {
         ...
         $userData = parent::selectFixUser(get_current_user_id());
         $formField = parent::getAttachedFieldsArray(FORM_ID_REGISTERED);
         $data = new CustomContactFormsUserData(array('encoded_data' => $userData->fix_data));

         ?>
<h3 class="hndle">

   <span>
              <?php _e("Biodata Pribadi", 'custom-contact-forms'); ?>
              </span>
...
<table class="widefat post" id="form-submissions-table" cellspacing="0">
   ...
   <tbody>
      <tr>
         <td scope="col" class="manage-column ccf-width250"><?php _e("Terdaftar Sejak", 'custom-contact-forms'); ?></td>
         <td><?php echo  esc_html(date('F d, Y h:i:s A', strtotime($user_info->user_registered))); ?></td>
      </tr>
            <?php
            ...
            $i = 0;
            foreach ($data_array as $item_value) {
               $field = parent::selectField($formField[$i]);
               $i++;
               ?>
               <tr>
         <td>
         ...
         <td>
                ...
                if (strpos($item_value, "[file link") !== false) {
                     if (strpos($data->parseUserData($item_value), ".jpg") !== false ||
                           strpos($data->parseUserData($item_value), ".jpeg") !== false ||
                           strpos($data->parseUserData($item_value), ".png") !== false ||
                           strpos($data->parseUserData($item_value), ".gif") !== false ||
...
jQuery(document).ready(function() {
   ...
   <div id="example_dashboard_widget" class="postbox" style="max-width:600px;padding:15px">
...
//echo do_shortcode("[custom form=question]");
         $this->printBoxUsers();
      }

      function printBoxUsers() {
         $user_data_array = parent::selectAllFixUsers();
         $formField = parent::getAttachedFieldsArray(FORM_ID_REGISTERED);

         foreach ($user_data_array as $userData) {

            $user_info = get_userdata($userData->user_id);
         ?>

<div id="ccf-form-success" style="display: none; width: 600px; height: 500px; top: 10%; z-index: 1;" class="fix-user-<?php echo $userData->user_id ?>">
   <div>
      <h5>Detil Pengguna</h5>
            <?php
            $data = new CustomContactFormsUserData(array('encoded_data' => $userData->fix_data));?>
            <a href="javascript:void(0)" class="close fix-user-close">×</a>
   </div>
   <p>
