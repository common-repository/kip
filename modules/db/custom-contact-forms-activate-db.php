...
if (!class_exists('CustomContactFormsActivateDB')) {
   class CustomContactFormsActivateDB extends CustomContactFormsDB {
      ...
      function fixDataTableExists() {
         global $wpdb;
         return ($wpdb->get_var("show tables like '". CCF_FIX_DATA_USER_TABLE . "'") == CCF_FIX_DATA_USER_TABLE);
      }

      function questionTableExists() {
         global $wpdb;
         return ($wpdb->get_var("show tables like '". CCF_QUESTION_TABLE . "'") == CCF_QUESTION_TABLE);
      }
      ...
      function createTables() {
         ...
         if(!$this->fixDataTableExists()) {
            $sql4 = "CREATE TABLE `".CCF_FIX_DATA_USER_TABLE."` (
                  `id` INT( 11 ) NOT NULL AUTO_INCREMENT ,
                  `user_id` VARCHAR( 30 ) NOT NULL ,
                  `fix_data` TEXT NOT NULL DEFAULT '',
                  PRIMARY KEY ( `id` )
                  ) ENGINE = MYISAM AUTO_INCREMENT=1 ";
            $wpdb->query($sql4);
         }

         if(!$this->questionTableExists()) {
            $sql5 = "CREATE TABLE `".CCF_QUESTION_TABLE."` (
                  `id` INT( 11 ) NOT NULL AUTO_INCREMENT ,
                  `user_id` VARCHAR( 30 ) NOT NULL ,
                  `data_time` INT( 11 ) NOT NULL DEFAULT '0',
      ...
      function updateTables() {
         ...
         if (!$this->columnExists('reject_data', CCF_QUESTION_TABLE))
            $wpdb->query("ALTER TABLE `" . CCF_QUESTION_TABLE . "` ADD `reject_data` TEXT DEFAULT ''");
         if (!$this->columnExists('info_source', CCF_QUESTION_TABLE))
            $wpdb->query("ALTER TABLE `" . CCF_QUESTION_TABLE . "` ADD `info_source` TEXT DEFAULT ''");

         if (!$this->columnExists('action_time', CCF_QUESTION_TABLE))
            $wpdb->query("ALTER TABLE `" . CCF_QUESTION_TABLE . "` ADD `action_time` INT( 11 ) NOT NULL DEFAULT '" . time() . "'");
      ...
      function insertFixedFields() {
         $captcha = array (
               'field_slug' => 'captcha',
               'field_label' => __ ( 'Type the numbers.', 'custom-contact-forms' ),
               'field_type' => 'Text',
               'field_value' => '',
               'field_maxlength' => '100',
               'user_field' => 0,
               'field_instructions' => __ ( 'Type the numbers displayed in the image above.', 'custom-contact-forms' )
         );
         $fixedEmail = array (
               'field_slug' => 'fixedEmail',
               'field_required' => 1,
               'field_label' => __ ( 'Email', 'custom-contact-forms' ),
               'field_type' => 'Text',
               'field_value' => '',
               'field_maxlength' => '100',
               'user_field' => 0,
               'field_instructions' => __ ( 'Silakan masukkan alamat email.', 'custom-contact-forms' )
         );
