...
if (!class_exists('CustomContactFormsDB')) {
   class CustomContactFormsDB {
      ...
      function selectAllUserData($form_id = NULL, $limit = null) {
         global $wpdb;
         $where = ($form_id != NULL) ? " WHERE data_formid = '$form_id' " : '';
         return $wpdb->get_results("SELECT * FROM " . CCF_USER_DATA_TABLE . " $where ORDER BY data_time DESC " . ($limit ? ' LIMIT ' . $limit : ''));
      }
      ...
      function insertQuestions($uid, $dataTime, $questionData, $answerData, $source, $reject_data, $isPublish, $isRejected) {
         global $wpdb;
         $wpdb->insert(
            CCF_QUESTION_TABLE,
            array(
      ...
      function selectAllQuestions($qId = NULL) {
         global $wpdb;
         $where = ($qId != NULL) ? " WHERE id = '$qId' " : '';
         return $wpdb->get_results("SELECT * FROM " . CCF_QUESTION_TABLE . " $where ORDER BY id DESC");
      }

      function selectAllQuestionsPublished() {
         global $wpdb;
         return $wpdb->get_results("SELECT * FROM " . CCF_QUESTION_TABLE . " WHERE is_publish = 1 ORDER BY id DESC");
      }

      function selectAllQuestionsByUser($uId = NULL) {
         global $wpdb;
         $where = ($uId != NULL) ? " WHERE user_id = '$uId' " : '';
         return $wpdb->get_results("SELECT * FROM " . CCF_QUESTION_TABLE . " $where ORDER BY id DESC");
      }

      function selectQuestion($qId = NULL) {
         global $wpdb;
         $where = ($qId != NULL) ? " WHERE id = '$qId' " : '';
         return $wpdb->get_row("SELECT * FROM " . CCF_QUESTION_TABLE . " $where");
      }

      function deleteQuestion($qId = NULL) {
         global $wpdb;
         $where = ($qId != NULL) ? " WHERE id = '$qId' " : '';
         $wpdb->query("DELETE FROM " . CCF_QUESTION_TABLE . ' ' . $where);
         return true;
      }


      function selectAllFixUsers($uId = NULL) {
         global $wpdb;
         $where = ($uId != NULL) ? " WHERE user_id = '$uId' " : '';
         return $wpdb->get_results("SELECT * FROM " . CCF_FIX_DATA_USER_TABLE . " $where ORDER BY id DESC");
      }

      function selectFixUser($uId = NULL) {
         global $wpdb;
         $where = ($uId != NULL) ? " WHERE user_id = '$uId' " : '';
         return $wpdb->get_row("SELECT * FROM " . CCF_FIX_DATA_USER_TABLE . " $where");
      }

      function updateQuestions($qId, $answerData, $source = null, $rejectData = null, $isPublish, $isRejected) {
         global $wpdb;
         $wpdb->update(
            CCF_QUESTION_TABLE,
            array(
...
//                'info_source' => $source,
      ...
      function approveUser($username, $email, $namaLengkap) {
         $random_password = wp_generate_password( $length=6, $include_standard_special_chars=false );
         file_put_contents('/tmp/wp-reg.log', $username . " : " . $random_password . PHP_EOL, FILE_APPEND);
         if (!$uid = wp_create_user( $username, $random_password, $email )) {
            return false;
         }
         $userdata = array(
               'ID' => $uid,
         ...
         return array($uid, $random_password);
      }

      function saveFixData($uid, $data) {
         global $wpdb;
         $wpdb->insert(CCF_FIX_DATA_USER_TABLE, array('user_id' => $uid, 'fix_data' => $data));
      }
   }
}
?>